...
void main(void)
{
	...
	float dotProductDiffuse = dot(n_norm, l);
	color_diffuse = k_diff * dotProductDiffuse;
	...
	//				uniform.
	vec3 cameraWorld = vec3(cameraMatrixInverse * vec4(0, 0, 0, 1));
	vec3 v = cameraWorld - worldPosition;
	v = normalize(v);

	vec3 r = ((2.0 * dot(n_norm, l)) * n_norm) - l; 

	float dotProductSpecular = pow(dot(v, r), shiny);

	color_specular = k_spec * dotProductSpecular;
