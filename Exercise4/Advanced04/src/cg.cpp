...
void CG::update(float dt)
{
    ...
    for(Particle& p : particles)
    {
        ...
        if (p.lifeTime < 0) {
            p.position = particleStart;
            p.lifeTime = glm::linearRand(1.0, 10.0);
            p.timeOffset = glm::linearRand(0.1, 1.0);
            vec3 smallRandomVector = glm::linearRand(vec3(0,0,0), vec3(0.5,0.5,0.5));
            p.velocity = (planeNormal + smallRandomVector) * glm::linearRand(2.0, 5.0);
        }
        else {
...
void CG::renderParticles()
{
    ...
    for(Particle& p : particles)
    {
        ...
        mat4 rotation = glm::rotate(0.5f * glm::pi<float>(), vec3(1, 0, 0));
        mat4 viewMatrix = camera.getViewMatrix();
        mat3 viewMatrixRotPart = mat3(viewMatrix);
        viewMatrixRotPart = glm::inverse(viewMatrixRotPart);
        mat4 viewMatrixApply = mat4(viewMatrixRotPart);
        mat4 particleTransformation = glm::translate(p.position) * viewMatrixApply * rotation; // <- Change this line
...
static mat3 orthonormalBasis(vec3 dir)
{
    ...
    vec3 firstVector = normalize(vec3(0, -normalizedDir[2], normalizedDir[1]));
    vec3 secondVector = normalize(cross(normalizedDir, firstVector));
