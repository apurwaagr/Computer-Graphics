...
function Basic1_2(canvas) {
    ...
    function PerspectiveProjection2D(eye, imagePlane, point2D) {
        ...
        // camera space
        imagePlane_old = imagePlane
        //console.log(eye[0] + " " + eye[1]);
        imagePlane = imagePlane - eye[1];
        point2D = [point2D[0] - eye[0], point2D[1] - eye[1]];
       // console.log(point2D[0] + " " + point2D[1]);

        // perspective projection
        let result = point2D[0] / (point2D[1] / imagePlane);
       // console.log(result);
        return result;
...
class Camera {
    ...
    update() {
        ...
        // camera matrix
        let w = negViewDir;
        let u = [negViewDir[1], -negViewDir[0]];
        let e = [this.eye[0], this.eye[1]];

        this.cameraMatrix[0] = u[0];
        this.cameraMatrix[1] = w[0];
        this.cameraMatrix[2] = 0;

        this.cameraMatrix[3] = u[1];
        this.cameraMatrix[4] = w[1];
        this.cameraMatrix[5] = 0;

        this.cameraMatrix[6] = -(u[0] * e[0] + u[1] * e[1]);
        this.cameraMatrix[7] = -(w[0] * e[0] + w[1] * e[1]);
    ...
    projectPoint(point2D) {
        ...
        let homogeneousPoint = [point2D[0], point2D[1], 1]
        let projectionAndCamera = mat3.create();
        mat3.multiply(projectionAndCamera, this.projectionMatrix, this.cameraMatrix);
        let result = [projectionAndCamera[0] * homogeneousPoint[0] + projectionAndCamera[3] * homogeneousPoint[1] + projectionAndCamera[6] * homogeneousPoint[2],
                projectionAndCamera[1] * homogeneousPoint[0] + projectionAndCamera[4] * homogeneousPoint[1] + projectionAndCamera[7] * homogeneousPoint[2],
                projectionAndCamera[2] * homogeneousPoint[0] + projectionAndCamera[5] * homogeneousPoint[1] + projectionAndCamera[8] * homogeneousPoint[2]];
