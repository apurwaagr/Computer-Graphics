...
function scanline(image, polygon) {
    ...
    for (let y_scanline = 0; y_scanline < image.height; y_scanline++) {
        ...
        /*if (activeEdgeTable.nEntries === 0){
            y_scanline = edgeTable.entries[edgeTable.entries.length-1].y_lower;
        }*/
        ...
        for (let entry of activeEdgeTable.entries) {
            if (entry.y_upper >= y_scanline) {
                newActiveEdgeTable.entries.push(entry);
        ...
        for (let edge of edgeTable.entries) {
            if (edge.y_lower === y_scanline) {
                activeEdgeTable.entries.push(new ActiveEdgeTableEntry(edge));
        ...
        for (var i = 0; i < activeEdgeTable.nEntries; i = i + 2){
            let x = Math.round(activeEdgeTable.entries[i].x_intersect);

            for (var j = x; j < activeEdgeTable.entries[i + 1].x_intersect; j++){
                setPixel(image, new Point(j, y_scanline), new Color(0, 0, 0));
            }
        }
        ...
        for (let entry of activeEdgeTable.entries){
            entry.x_intersect = entry.x_intersect + entry.invSlope;
        }
