...
class DepthBuffer {
    ...
    TestAndSetFragment(x, valueF, depthTestMode) {
        ...
        if (depthTestMode == 0 || (depthTestMode == 1 && newValue < oldValue)) {
            this.data[x] = newValue;
            return true;
        }
        if (depthTestMode == -1 && newValue > oldValue) {
            this.data[x] = newValue;
            return true;
        }
        return false;
...
class RenderingPipeline {
    ...
    PrimitiveAssemblyStage(vertexStream, ibo) {
        ...
        let primitives = new Array(ibo.length / 2); // Also change the size of this array.
        let p = 0;

        for (let i = 0; i < ibo.length - 1; i++) {
            let idx_a = ibo[i];
            let idx_b = ibo[i + 1];
            primitives[p] = [vertexStream[idx_a], vertexStream[idx_b]];
            p++;
        }
    ...
    LineCulling(a, b) {
        ...
        if (this.culling == 0) {
            ...
            let line = [b[0] / b[2] - a[0] / a[2], b[1] / b[2]- a[1] / a[2]];
            let normal = [line[1], - line[0]];
            ...
            let cameraMatrixInverse = mat3.create();
            mat3.invert(cameraMatrixInverse, this.uniforms.cameraMatrix);
            let viewDir = [-cameraMatrixInverse[3], -cameraMatrixInverse[4]];
            vec2.normalize(viewDir, viewDir);

            if (vec2.dot(viewDir, normal) > 0) {
                if (this.culling == 1) {
                    return true;
                } else {
                    return false;
