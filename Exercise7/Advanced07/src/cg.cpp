...
void CG::decoupledMainLoop()
{
    ...
    while (running) {
        ...
        Uint64 ticksPerUpdate = (1 / updateRate) * SDL_GetPerformanceFrequency();
        Uint64 ticksPerRender = (1 / frameRate) * SDL_GetPerformanceFrequency();
        ...
        if (nextRender < currentTime) {
            ...
            if (interpolationMethod == 0) // No interpolation
                ...
                alpha += 1;
            interpolate(1 / updateRate, alpha); // Change this line in order to use the correct dt.
            render();
            imgui.beginFrame();
            renderGui();
            imgui.endFrame();
        ...
        Uint64 nextEvent = min(nextUpdate, nextRender);
