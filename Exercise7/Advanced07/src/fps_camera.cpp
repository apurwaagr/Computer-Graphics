...
void FPSCamera::turn(vec2 relMouseMovement)
{
    ...
    Quaternion xAxisQuat = Quaternion(vec3(0,1,0), dx * 0.1 * glm::pi<float>());

    Quaternion yAxisQuat = Quaternion(vec3(1, 0, 0), dy * 0.1 * glm::pi<float>());

    //Quaternion movement;
    //movement.real = 0;
    //movement.img = vec3(dy, dx, 0);

    //movement = yAxisQuat * movement;


    // compute newOrientation from currentTransformation.orientation
    Quaternion newOrientation = xAxisQuat * currentTransformation.orientation;
    newOrientation = newOrientation * yAxisQuat;

    if ((newOrientation * vec3(0, 1, 0)).y <= 0) {
        newOrientation = xAxisQuat * currentTransformation.orientation;
    }
...
void FPSCamera::updatePosition(float dt)
{
    ...
    if (keyBoardState[SDL_SCANCODE_SPACE] && y == this->startY) {
        this->vy = 5;
    }
    y += vy * dt;
    this->vy += (-9.81) * dt;
